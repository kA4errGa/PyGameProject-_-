...
class Parametrs:
    ...
    def paint_par(self, s):
        ...
        self.text_life_name = (self.par_life_font).render(f'Жизнь: {self.param_life}/{self.param_life_max}', True, 'red')
        s.blit(self.text_life_name, (14, 60))
        self.par_power_font = font.Font('Merriweather/Merriweather-Regular.ttf', 20)
        self.text_power_name = (self.par_power_font).render(f'Энергия: {self.param_power}/{self.param_power_max}', True, 'blue')
        s.blit(self.text_power_name, (14, 160))
        self.par_mony_font = font.Font('Merriweather/Merriweather-Regular.ttf', 24)
        self.text_mony_name = (self.par_mony_font).render(f'Деньги: {self.param_mony}/{self.param_mony_max}', True, 'yellow')
